...
def build_report_pdf(metrics: dict) -> bytes:
    ...
    story = [
        ...
        Paragraph("The evaluation is designed to show whether the proposed agent adds value beyond simple label heuristics. The majority baseline always predicts billing and reaches 10% on the balanced set. The keyword baseline checks whether at least one gold-intent keyword appears and reaches 64% coverage. The proposed agent combines phrase-aware rules, historical overlap, intent-restricted retrieval, and conservative routing.", body),
    ]
    metrics_table = Table([
